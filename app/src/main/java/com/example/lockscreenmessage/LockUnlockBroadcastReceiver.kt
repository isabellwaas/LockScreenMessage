    ...
    override fun onReceive(context: Context?, intent: Intent?)
    {
        if(context != null)
        {
            ...
            if (intent?.getAction().equals(Intent.ACTION_SCREEN_OFF) && this.keyguardManager.isDeviceLocked)
            {
                ...
                if(!title.isBlank() || !text.isBlank()) this.singleNotificationHelper.sendNotificationIfNonePresent(context,context.getString(com.example.lockscreenmessage.R.string.lock_screen_channel_id), persistentSaver.readValue(context.getString(com.example.lockscreenmessage.R.string.lock_screen_message_id), 11223344), title, text, NotificationManager.IMPORTANCE_MAX, R.drawable.ic_notification)
            }
